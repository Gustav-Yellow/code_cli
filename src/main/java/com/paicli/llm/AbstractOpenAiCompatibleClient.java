 ...
 * OpenAI 兼容 API 的抽象基类。
 * <p>
 * 封装了 HTTP 请求构建、SSE（Server-Sent Events）流式解析、tool_call 增量合并等通用逻辑。
 * 子类只需实现 {@link #getApiUrl()}、{@link #getModel()}、{@link #getApiKey()} 三个方法，
 * 以及 {@link #getModelName()}、{@link #getProviderName()} 两个标识方法。
 */
public abstract class AbstractOpenAiCompatibleClient implements LlmClient {
    ...
    /**
     * 从 OpenAI 兼容协议的 usage 中提取 prompt cache 命中 token 数。
     * 不同 provider 使用不同字段名，逐字段尝试。
     */
