...
public class AgentOrchestrator {
    ...
    public AgentOrchestrator(String apiKey) {
        this(new GLMClient(apiKey), new ToolRegistry(), new MemoryManager(new GLMClient(apiKey)), null);
    }

    public AgentOrchestrator(String apiKey, ToolRegistry toolRegistry) {
        this(new GLMClient(apiKey), toolRegistry, new MemoryManager(new GLMClient(apiKey)), null);
    }
    ...
    public AgentOrchestrator(String apiKey, ToolRegistry toolRegistry, MemoryManager memoryManager) {
        this(new GLMClient(apiKey), toolRegistry, memoryManager, null);
    }

    /**
     * 共享上下文构造器：注入会话级 sharedHistory，
    ...
    public String run(String userInput) {
        ...
        // 压缩共享历史（与 ReAct/Plan 同一调用，作用于会话级历史），并取先前对话上下文供规划参考
        ...
        // 检索相关长期记忆并注入到规划消息中（与 ReAct/Plan 模式一致的记忆接入方式）
        ...
        String planInput = "请为以下任务制定执行计划：\n" + userInput;
        if (priorContext != null && !priorContext.isEmpty()) {
            planInput = priorContext + "\n\n" + planInput;
        }
        if (memoryContext != null && !memoryContext.isEmpty()) {
            planInput = memoryContext + "\n\n" + planInput;
        }
        AgentMessage planMessage = AgentMessage.task("orchestrator", planInput);
