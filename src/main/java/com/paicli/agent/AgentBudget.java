 ...
 * Agent 循环的退出预算。
 *
 * <p>设计目标是把"是否继续下一轮"的主导权交给 LLM 自己——只要它返回 content 不再调用工具，
 * 循环就退出。本类只承担三种"保险阀"职责，避免模型在异常情况下无限烧 token：
 *
 * <ol>
 *   <li>Token 预算：累计 input + output token 超过阈值后强制收尾</li>
 *   <li>停滞检测：连续 N 次工具调用使用完全相同的工具名 + 参数，判定为死循环</li>
 *   <li>硬轮数兜底：累计迭代轮数超过 hardMaxIterations，作为兜底防御</li>
 * </ol>
 *
 * <p>配置读取顺序（以 {@link #fromSystemProperties()} 为准）：
 * <ol>
 *   <li>系统属性：{@code paicli.react.token.budget} / {@code paicli.react.stagnation.window} /
 *       {@code paicli.react.hard.max.iterations}</li>
 *   <li>默认值：300_000 token / 连续 3 次相同工具调用 / 50 轮</li>
 * </ol>
 */
public class AgentBudget {
    ...
    /** 按当前 LLM 模型的 maxContextWindow 动态计算 token 预算（默认 80% × window） */
     ...
     * 记录本轮工具调用签名并判断是否进入停滞。
     *
     * <p>停滞条件：最近 stagnationWindow 轮的"工具名 + 参数"完全相同；
     * 一旦判定为停滞，状态会保持。
     */
